...
var outline = new Path.Rectangle(symbolGroup.bounds.scale(1.3), new Size(20,20));
...
var arrPath = new Path([new Point(-radius/2,0), new Point(radius/2,0), new Point(radius/4, -radius/4)]);
...
for (var i=0; i < numSides; i++) {
  ...
  rLine.onMouseEnter = function(){this.strokeColor = "#08fff7"};
  rLine.onMouseLeave = function(){this.strokeColor = "#ccc"};
  reflectionGroup.addChild(rLine);
  if (i < numSides - 1) { // Exit early if it's the last reflection line.
